...
**התקן מוזיקלי פסיכודלי מקצה לקצה — לב קצב, סינתזה, מסע ויזואלי, רשת משפחתית ונהגים אמיתיים במערכת קנונית אחת**
...
![phases](https://img.shields.io/badge/phases-6%2F6-ffd166?style=for-the-badge)
...
הממשק (`web/index.html`) הוא **מסע DMT אינטראקטיבי**: קיילודוסקופ חי, מנדלה נושמת, טבעת ספקטרום, ארבעה ערוצי סינתזה, זיכרון פורטל, חלימה בין עולמות, אלכימיית קצב אוקלידית, סנכרון רב-מכשירי, הקלטת מסעות ויציאת MIDI — הכול בדפדפן.
...
להקלטה: הפעילו את המסע, לחצו **הקלטת מסע ●**, נגנו, עצרו — והמסע יורד כקובץ.

ל-MIDI: חברו סינתזה חיצונית, לחצו **יציאת MIDI** — וכל צעד ישלח החוצה כתווים אמיתיים.

## ✦ מבנה

- `docs/` — ארכיטקטורה, roast, החלטות, בדיקות, עיצוב, אינטגרציה, מפת דרכים
- `src/` — core · domain · protocol · engine · integration · ui
- `tests/` — unit · domain · protocol · engine · integration · drivers
...
**PSYSTAR — לא מכשיר. שער. צומת של רשת. ועתה — כלי שמנגן עם כל החדר.**
