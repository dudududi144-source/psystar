...
**התקן מוזיקלי פסיכודלי מקצה לקצה — לב קצב, סינתזה, מסע ויזואלי, רשת משפחתית, נהגים אמיתיים וזיכרון מסעות במערכת קנונית אחת**
...
![phases](https://img.shields.io/badge/phases-7%2F7-ffd166?style=for-the-badge)
...
- **Domain** — המודל המוזיקלי: תבניות, תחבורה, התקנים, סצנות, שירים, קצב אוקלידי, פריסטים, יומני מסע.
...
הממשק (`web/index.html`) הוא **מסע DMT אינטראקטיבי**: קיילודוסקופ חי, מנדלה נושמת, טבעת ספקטרום, ארבעה ערוצי סינתזה, זיכרון פורטל, חלימה בין עולמות, אלכימיית קצב אוקלידית, סנכרון רב-מכשירי, הקלטת מסעות, יציאת MIDI — ועתה גם יומן מסע שמנגנים אותו חזרה.
...
- יצוא סצנות כ-JSON ויבוא חזרה — שיתוף בין מכשירים
...
לסנכרון רב-מכשירי: פתחו את `web/index.html` בשני טאבים. באחד לחצו **הפוך למוביל**, בשני **עקוב אחרי מוביל**.

לזיכרון מסע: חמשו **יומן מסע ●**, נגנו, ואז לחצו **ניגון יומן ✦** — המכונה תיזכר ותנגן אתכם שוב.

לשיתוף סצנות: **יצוא סצנה ⬇** מוריד JSON; **יבוא סצנה ⬆** טוען אותו בכל מכשיר אחר.

## ✦ מבנה

- `docs/` — ארכיטקטורה, roast, החלטות, בדיקות, עיצוב, אינטגרציה, מפת דרכים
- `src/` — core · domain · protocol · engine · integration · ui
- `tests/` — unit · domain · protocol · engine · integration · drivers · journey
...
**PSYSTAR — לא מכשיר. שער. צומת של רשת. כלי שמנגן עם כל החדר. ומכונה שזוכרת כל מסע.**
