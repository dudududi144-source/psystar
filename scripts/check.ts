import { existsSync } from 'node:fs';

const requiredFiles = [
  'package.json',
  'README.md',
  'tsconfig.json',
  'src/index.ts',
  'src/main.ts',
  'src/app.ts',
  'src/core/result.ts',
  'src/core/event-bus.ts',
  'src/core/history.ts',
  'src/domain/pattern.ts',
  'src/domain/scene.ts',
  'src/domain/song.ts',
  'src/domain/euclidean.ts',
  'src/domain/preset.ts',
  'src/domain/journey.ts',
  'src/domain/library.ts',
  'src/domain/accent.ts',
  'src/domain/harmony.ts',
  'src/domain/evolution.ts',
  'src/domain/pack.ts',
  'src/domain/velocity.ts',
  'src/domain/roll.ts',
  'src/domain/groove.ts',
  'src/domain/probability.ts',
  'src/domain/variation.ts',
  'src/domain/voicing.ts',
  'src/domain/roll-transform.ts',
  'src/domain/progression-gen.ts',
  'src/domain/progression-store.ts',
  'src/domain/groove-template.ts',
  'src/domain/auto-dream.ts',
  'src/domain/legato.ts',
  'src/domain/mastering-preset.ts',
  'src/domain/setlist.ts',
  'src/domain/melody-gen.ts',
  'src/domain/tempo-ramp.ts',
  'src/domain/melody-variation.ts',
  'src/domain/song-builder.ts',
  'src/domain/journey-timeline.ts',
  'src/protocol/codec.ts',
  'src/engine/scheduler.ts',
  'src/engine/lookahead-scheduler.ts',
  'src/engine/envelope.ts',
  'src/engine/voice-manager.ts',
  'src/engine/effects-rack.ts',
  'src/engine/foundation-adapter.ts',
  'src/engine/midi-driver.ts',
  'src/engine/recorder.ts',
  'src/engine/tap-tempo.ts',
  'src/engine/humanizer.ts',
  'src/engine/wav-encoder.ts',
  'src/engine/transcribe.ts',
  'src/engine/autopilot.ts',
  'src/engine/midi-clock.ts',
  'src/engine/song-render.ts',
  'src/engine/session.ts',
  'src/engine/metronome.ts',
  'src/engine/arpeggiator.ts',
  'src/engine/midi-melody.ts',
  'src/engine/midi-file.ts',
  'src/engine/mastering.ts',
  'src/engine/metering.ts',
  'src/engine/stems.ts',
  'src/engine/eq.ts',
  'src/engine/midi-learn.ts',
  'src/engine/midi-input-map.ts',
  'src/integration/foundation-bridge.ts',
  'src/integration/device-registry.ts',
  'src/integration/sync-protocol.ts',
  'src/integration/p2p-signaling.ts',
  'src/integration/foundation-protocol.ts',
  'src/integration/foundation-device.ts',
  'tests/core.test.ts',
  'tests/integration.test.ts',
  'tests/scene.test.ts',
  'tests/lookahead.test.ts',
  'tests/envelope.test.ts',
  'tests/voice-manager.test.ts',
  'tests/effects-rack.test.ts',
  'tests/song.test.ts',
  'tests/euclidean.test.ts',
  'tests/preset.test.ts',
  'tests/journey.test.ts',
  'tests/library.test.ts',
  'tests/pwa.test.ts',
  'tests/tap-tempo.test.ts',
  'tests/history.test.ts',
  'tests/midi-input-map.test.ts',
  'tests/humanizer.test.ts',
  'tests/accent.test.ts',
  'tests/harmony.test.ts',
  'tests/evolution.test.ts',
  'tests/pack.test.ts',
  'tests/velocity.test.ts',
  'tests/roll.test.ts',
  'tests/wav-encoder.test.ts',
  'tests/groove.test.ts',
  'tests/probability.test.ts',
  'tests/variation.test.ts',
  'tests/spectrum.test.ts',
  'tests/voicing.test.ts',
  'tests/arpeggiator.test.ts',
  'tests/midi-melody.test.ts',
  'tests/midi-file.test.ts',
  'tests/roll-transform.test.ts',
  'tests/progression-gen.test.ts',
  'tests/progression-store.test.ts',
  'tests/groove-template.test.ts',
  'tests/auto-dream.test.ts',
  'tests/legato.test.ts',
  'tests/mastering-preset.test.ts',
  'tests/setlist.test.ts',
  'tests/melody-gen.test.ts',
  'tests/tempo-ramp.test.ts',
  'tests/melody-variation.test.ts',
  'tests/song-builder.test.ts',
  'tests/journey-timeline.test.ts',
  'tests/help.test.ts',
  'tests/mastering.test.ts',
  'tests/metering.test.ts',
  'tests/stems.test.ts',
  'tests/waterfall.test.ts',
  'tests/eq.test.ts',
  'tests/transcribe.test.ts',
  'tests/autopilot.test.ts',
  'tests/midi-clock.test.ts',
  'tests/song-render.test.ts',
  'tests/session.test.ts',
  'tests/metronome.test.ts',
  'tests/midi-learn.test.ts',
  'tests/foundation-bridge.test.ts',
  'tests/device-registry.test.ts',
  'tests/sync-protocol.test.ts',
  'tests/p2p-signaling.test.ts',
  'tests/foundation-protocol.test.ts',
  'tests/foundation-device.test.ts',
  'tests/midi-driver.test.ts',
  'tests/recorder.test.ts',
  'web/index.html',
  'web/manifest.webmanifest',
  'web/sw.js',
  'web/icons/icon-192.png',
  'web/icons/icon-512.png',
  'docs/design.md',
  'docs/integration.md'
];

let failed = false;

for (const file of requiredFiles) {
  if (!existsSync(file)) {
    console.error('missing required file: ' + file);
    failed = true;
  }
}

if (failed) {
  process.exit(1);
}

console.log('structure check passed - PSYSTAR skeleton is canonical');
